...
class Carrito(db.Model):
	...
	def convert(self, lista, cantidad):
		jsona = [{'id_user':lista[i].id_user,'id_prod':lista[i].id_prod,'name_prod':lista[i].nombre_producto,'price':lista[i].precio,'cant':lista[i].cantidad} for i in range(0,cantidad)]
		return jsona
